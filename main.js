// Magnum Opus — find your best online chess games, analysed in your browser.
    ...
    if (lichessUser) {
      ...
      const pgnText = await pgn.fetchGamesPGN(lichessUser, {
        ...
        onProgress: (received, games) => {
          ...
          stepBar('download', Math.min(chesscomUser ? 0.5 : 0.97, Math.max(byGames, byBytes)));
    ...
    if (chesscomUser) {
      ...
      const ccGames = await cc.fetchChesscomGames(chesscomUser, {
        ...
        onProgress: (n, done, total) => {
          const start = lichessUser ? 0.5 : 0.05;
          stepBar('download', Math.min(0.97, start + (0.97 - start) * (total ? done / total : 0)));
          const lc = counts.lichess || 0;
          stepStat('download', `${lc + n} games so far (Lichess ${lc}, Chess.com ${n})${total ? ` - month ${done}/${total}` : ''}`);
        },
      });
...
<title>${data.meta.username}'s best online chess games — Magnum Opus</title>
